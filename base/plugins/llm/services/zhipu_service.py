...
class ZhipuService:
    ...
    async def chat(self, model: str, messages: List[Dict],
        ...
        try:
            ...
            raise Exception(f"智谱AI API调用失败: {e.response.text}")
        except Exception as e:
            logger.error(f"智谱AI服务异常: {str(e)}")
            raise
    ...
    async def chat_stream(self, model: str, messages: List[Dict],
        ...
        try:
            ...
            logger.error(f"智谱AI流式API错误: {e.response.status_code}")
            raise Exception(f"智谱AI流式调用失败: {e.response.text}")
        except Exception as e:
            logger.error(f"智谱AI流式服务异常: {str(e)}")
            raise
